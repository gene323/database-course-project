...
<head>
	...
	<title>Edit industry academy cooperation project</title>
...
<body>
	...
	<div class="flex justify-center mt-5">
		<button type="button" onclick="toggleInsertModal()" id="insertBtn"
			class="w-96 h-8 bg-blue-300 hover:bg-blue-400">
			Insert a new industry academy cooperation project
		</button>
	</div>

	<!-- fetch data of industry academy cooperation projects -->
	<?PHP
	$sql = "SELECT * FROM industry_academy_cooperation_project";
	$res = $conn->query($sql);
	$i = 1;
	?>

	<!-- echo info of industry academy cooperation projects -->
		...
		while ($row = $res->fetch_assoc()) {
			...
			<div class="flex w-full h-24 gap-1">
				...
				<button class="bg-blue-200 rounded-md w-12 h-[90%] my-auto hover:bg-blue-400"
					onclick="toggleIndustryAcademyCooperationProject('<?php echo $row['ID'] ?>', '<?php echo $row['title'] ?>', '<?php echo $row['date_begin'] ?>', '<?php echo $row['date_end'] ?>', '<?php echo $row['position'] ?>')">
					Edit
				</button>
				<button type="submit"
					onclick="deleteRecord('../upload/industry-academy-cooperation-project.php', 'delete', '<?php echo $row['ID'] ?>')"
	...
	<!-- edit industry academy cooperation project modal -->
	<div id="editModal" class="hidden h-screen w-full fixed top-0 left-0 bg-black bg-opacity-50">
		<div class="bg-white relative h-80 w-2/3 rounded-md mx-auto top-10">
			...
			<form method="post" action="../upload/industry-academy-cooperation-project.php"
				...
				<div>
					<label>From: <input type="date" id="editDateBegin" name="date_begin" value=""
				...
				<div>
					<label>Position: <input type="text" id="editPosition" name="position" value=""
							class="border border-black">
					</label>
				</div>
	...
	<!-- insert industry academy cooperation project modal -->
